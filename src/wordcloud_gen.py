        ...
        if os.path.isdir(msg_dir):
            for f in os.listdir(msg_dir):
                ...
                if m:
                    ...
                    tables.append({"db_idx": int(m.group(1)), "db_path": db_path, "table_name": None})

    # Build time filter clause
    time_clause = ""
    if start_ts is not None:
        time_clause += f" AND create_time >= {start_ts}"
    if end_ts is not None:
        time_clause += f" AND create_time <= {end_ts}"
    ...
    for tinfo in tables:
        ...
        try:
            if tinfo["table_name"]:
                # Specific chat table
                query = f"""
                    SELECT message_content FROM [{tinfo['table_name']}]
                ...
                queries = " UNION ALL ".join(
                    f"SELECT message_content FROM [{t[0]}] WHERE local_type = 1 "
                    f"AND message_content IS NOT NULL AND create_time > 1000000000{time_clause}"
                    for t in msgtables
                )
                query = queries

            for (content,) in conn.execute(query):
                if content:
                    text = content
...
def _build_wordcloud_html(title, top_words, total_msgs, unique_words):
    ...
    word_data_json = str(word_data).replace("'", '"')
    bar_labels_json = str(bar_labels).replace("'", '"')
    bar_values_json = str(bar_values)
